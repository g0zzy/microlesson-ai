...
- **3 Learning Styles**:
  ...
  - 🎧 **Voice**: Listen to audio narration generated by ElevenLabs
  - 📊 **Visual**: Navigate through 5 interactive slides with OpenAI-generated images
...
- **OpenAI Images API**: Server-side image generation for visual lessons
...
- **Visual**: JSON array of exactly 5 slides with titles, text, and image prompts
...
The visual feature uses the **OpenAI Images API**. Claude creates structured slides and short `image_prompt` values, then the backend generates slide illustrations with OpenAI and returns them as inline data URLs.
...
5. Add environment variables: `ANTHROPIC_API_KEY`, `ELEVENLABS_API_KEY`, `OPENAI_API_KEY`

#### Option 2: Railway.app
1. Create new project from GitHub
2. Add `ANTHROPIC_API_KEY`, `ELEVENLABS_API_KEY`, and `OPENAI_API_KEY` environment variables
3. Railway auto-detects Python and runs the app

#### Option 3: Fly.io
1. Install flyctl
2. Run `fly launch` in project directory
3. Set secrets: `fly secrets set ANTHROPIC_API_KEY=your_key ELEVENLABS_API_KEY=your_key OPENAI_API_KEY=your_key`
4. Deploy: `fly deploy`
...
Set these env vars to change voice behavior:
```bash
ELEVENLABS_VOICE_ID=JBFqnCBsd6RMkjVDRZzb
ELEVENLABS_MODEL_ID=eleven_multilingual_v2
```

### Customize Visual Model

Set these env vars to tune image generation:
```bash
...
### Voice not playing
- Check browser console and network tab for failed `/generate-lesson` responses
- Verify `ELEVENLABS_API_KEY` is set
- Ensure speaker volume is up

### Visual images unavailable
- Verify `OPENAI_API_KEY` is set
- Check backend logs for OpenAI image API errors
- Confirm your selected `OPENAI_IMAGE_MODEL` is enabled for your API key
...
- [ ] More visual styles and image controls
